...
NestableObject {
    ...
    readonly property var liveMonitorData: WM.compositor === "hyprland" ? HyprlandData.monitors.find(m => m.id === hyprMonitor?.id) : null
    ...
    readonly property bool currentWorkspaceNotFake: WM.compositor === "hyprland" ? (activeWindow?.activated ?? false) // Active empty workspace = fake. At least, that's how I like to call it.
    : true
