...
local user_opts = {
    ...
    font = "Roboto",              -- font for the OSC (default: mpv-osd-symbols or the one set in mpv.conf)

    -- Material Design 3 styling
    -- when enabled, applies the MD3 palette below and switches the OSC to a
    -- Material look: tonal button pills, a filled FAB play button, a rounded
    -- "surface" container, and an MD3 progress bar (split active/inactive
    -- tracks with a pill thumb in the middle).
    -- NOTE: toggling this at runtime requires a restart to fully restore colors.
    material3 = true,                     -- master toggle for Material Design 3 styling
    material3_fab = true,                  -- draw a filled FAB-style background behind the play/pause button
    material3_button_pills = true,         -- draw tonal background pills behind the control buttons
    material3_surface_radius = 16,         -- corner radius of the MD3 OSC surface container (0 to disable)
    material3_elevation = true,            -- add a soft shadow under the FAB/pills for elevation
    -- Material Design 3 palette (only used when material3 = yes)
    md3_primary = "#D0BCFF",               -- primary: seekbar progress, FAB, accents
    md3_on_primary = "#381E72",            -- on-primary: icon color drawn on the FAB
    md3_surface = "#1C1B1F",               -- surface: OSC background color
    md3_surface_variant = "#49454F",       -- surface-variant: inactive seekbar track
    md3_on_surface = "#E6E1E5",            -- on-surface: titles, time codes, default icons
    md3_secondary_container = "#4A4458",   -- secondary-container: tonal button pills
    md3_on_secondary_container = "#E8DEF8",-- on-secondary-container: icon color on tonal pills
...
local function set_osc_styles()
    ...
    -- Material Design 3: remap the relevant colors to the MD3 palette so the
    -- whole OSC shares a cohesive tonal scheme. Done here (rather than in the
    -- conf) so a single material3=yes switch is enough to get the MD3 look.
    if user_opts.material3 then
        user_opts.osc_color            = user_opts.md3_surface
        user_opts.seekbarfg_color      = user_opts.md3_primary
        user_opts.seekbarbg_color      = user_opts.md3_surface_variant
        user_opts.seek_handle_color    = user_opts.md3_primary
        user_opts.title_color          = user_opts.md3_on_surface
        user_opts.chapter_title_color  = user_opts.md3_on_surface
        user_opts.time_color           = user_opts.md3_on_surface
        user_opts.cache_info_color     = user_opts.md3_on_surface
        user_opts.side_buttons_color   = user_opts.md3_on_surface
        user_opts.middle_buttons_color = user_opts.md3_on_surface
        user_opts.playpause_color      = user_opts.md3_on_primary
        user_opts.hover_effect_color   = user_opts.md3_primary
        user_opts.nibble_color         = user_opts.md3_primary
        user_opts.nibble_current_color = user_opts.md3_on_surface
        -- MD3 feedback is a tonal state layer ("box") plus a subtle size bump.
        -- Drop "color"/"glow" so icons never recolor into their own background
        -- (e.g. the play icon on the primary-colored FAB).
        user_opts.hover_effect = "box,size"
    end
    ...
    osc_styles = {
        ...
        -- Material Design 3 backgrounds
        md3_fab = "{\\blur0\\bord0\\1c&H" .. osc_color_convert(user_opts.md3_primary) .. "&}",
        md3_pill = "{\\blur0\\bord0\\1c&H" .. osc_color_convert(user_opts.md3_secondary_container) .. "&}",
        md3_shadow = "{\\blur6\\bord0\\1c&H000000&}",
    }
end
...
local function prepare_elements()
    ...
    -- Material Design 3 draws its own split active/inactive tracks inside the
    -- slider, so the full-width background track boxes are dropped.
    local md3_drop = user_opts.material3
        and { seekbarbg = true, volumebarbg = true } or {}
    local elements2 = {}
    for _, element in pairs(elements) do
        if element.layout ~= nil and element.visible and not md3_drop[element.name] then
...
-- Material Design 3 seekbar geometry: gap (px) cleared on each side of the
-- thumb, and the half-width of the pill thumb itself.
local function md3_seek_metrics(element)
    local is_vol = element.name == "volumebar"
    local thumb_w = is_vol and 3 or 4
    local thumb_gap = is_vol and 4 or 6
    return thumb_w, thumb_gap, thumb_w / 2 + thumb_gap
end
...
local function draw_seekbar_handle(element, elem_ass, handle_x, handle_radius, anim_override, is_active)
    -- Material Design 3: vertical pill thumb sitting between the two tracks
    if user_opts.material3 then
        local elem_geo = element.layout.geometry
        local slider_lo = element.layout.slider
        local center_y = elem_geo.h / 2
        local thumb_w = md3_seek_metrics(element)
        local thumb_h = elem_geo.h * (is_active and 1.15 or 1.0)
        local hx1 = limit_range(thumb_w / 2, elem_geo.w - thumb_w / 2, handle_x)
        begin_draw_layer(element, elem_ass, slider_lo.handle_color or user_opts.seekbarfg_color, anim_override)
        elem_ass:round_rect_cw(hx1 - thumb_w / 2, center_y - thumb_h / 2,
            hx1 + thumb_w / 2, center_y + thumb_h / 2, thumb_w / 2)
        return
    end
...
local function draw_seekbar_ranges(element, elem_ass, xp, rh, override_alpha, inverse)
    -- Material Design 3 keeps a clean two-track look without cache overlays
    if user_opts.material3 then return end
...
local function draw_seekbar_nibbles(element, elem_ass)
    -- Material Design 3 uses clean split tracks; skip chapter nibbles
    if user_opts.material3 then return end
...
local function draw_seekbar_progress(element, elem_ass)
    ...
    -- Material Design 3: two separate fully-rounded tracks (active + inactive)
    -- with a gap on each side of the thumb that sits between them.
    if user_opts.material3 and element.name ~= "persistent_seekbar" then
        local track_top = slider_lo.gap
        local track_bot = elem_geo.h - slider_lo.gap
        local tr = (track_bot - track_top) / 2 -- fully rounded track ends
        local _, _, clear = md3_seek_metrics(element)
        local left_end = xp - clear
        local right_start = xp + clear

        -- active track (primary)
        begin_draw_layer(element, elem_ass, slider_lo.handle_color or user_opts.seekbarfg_color)
        if left_end > track_top then
            local r = math.min(tr, (left_end) / 2)
            elem_ass:round_rect_cw(0, track_top, left_end, track_bot, r)
        end
        -- inactive track (surface-variant)
        begin_draw_layer(element, elem_ass, user_opts.seekbarbg_color)
        if right_start < elem_geo.w - track_top then
            local r = math.min(tr, (elem_geo.w - right_start) / 2)
            elem_ass:round_rect_cw(right_start, track_top, elem_geo.w, track_bot, r)
        end
        return
    end
...
local function render_elements(master_ass, osc_vis, wc_vis)
    local function render_element(n)
        ...
        -- Material Design 3: persistent button backgrounds (FAB / tonal pills),
        -- drawn first so they sit underneath the state layer and the icon.
        if user_opts.material3 and element.type == "button" then
            local n = element.name
            local md3_excluded = {
                title = true, chapter_title = true, time_codes = true,
                cache_info = true, speed = true, windowtitle = true,
                close = true, maximize = true, minimize = true,
            }
            local is_clickable_btn = element.eventresponder and (
                element.eventresponder["mbtn_left_down"] ~= nil or
                element.eventresponder["mbtn_left_up"] ~= nil)
            local kind
            if n == "play_pause" and user_opts.material3_fab then
                kind = "fab"
            elseif user_opts.material3_button_pills and is_clickable_btn
                and element.enabled and not md3_excluded[n] then
                kind = "pill"
            end

            if kind then
                -- start a clean event boundary so this elem_ass doesn't get
                -- concatenated onto the previous element's last line (assdraw
                -- merge does not insert a newline) — same guard the hover box uses
                elem_ass:append("{}")
                local hx1, _, hx2, _ = get_element_hitbox(element)
                local cx = (hx1 + hx2) / 2
                local cy = (element.hitbox.y1 + element.hitbox.y2) / 2
                -- size the background from the button's own geometry so it fits
                -- every layout (default, compact, mini, seekbar)
                local bw = hx2 - hx1
                local bh = element.hitbox.y2 - element.hitbox.y1
                local half_w, half_h, radius, bg_style
                if kind == "fab" then
                    half_h = bh / 2 + 10
                    half_w = math.max(half_h, bw / 2 + 6)
                    radius = math.min(user_opts.material3_surface_radius, half_h)
                    bg_style = osc_styles.md3_fab
                else
                    half_h = bh / 2 + 5
                    half_w = bw / 2 + 7
                    radius = half_h
                    bg_style = osc_styles.md3_pill
                end

                -- soft drop shadow for elevation
                if user_opts.material3_elevation then
                    elem_ass:new_event()
                    elem_ass:pos(0, 0)
                    elem_ass:an(7)
                    ass_append_alpha(elem_ass, {[1] = 170, [2] = 255, [3] = 255, [4] = 255}, element.layout.alpha[1], nil, anim_override)
                    elem_ass:append(osc_styles.md3_shadow)
                    elem_ass:draw_start()
                    elem_ass:round_rect_cw(cx - half_w, cy - half_h + 3, cx + half_w, cy + half_h + 3, radius)
                    elem_ass:draw_stop()
                end
                ...
                elem_ass:an(7)
                ass_append_alpha(elem_ass, element.layout.alpha, 0, nil, anim_override)
                elem_ass:append(bg_style)
                elem_ass:draw_start()
                elem_ass:round_rect_cw(cx - half_w, cy - half_h, cx + half_w, cy + half_h, radius)
                elem_ass:draw_stop()
            end
        end
...
local function setup_bg_elements(posX, posY, osc_w, osc_h, osc_alpha3, wc_alpha3)
    ...
    -- Material Design 3: round the top corners of the surface container
    if user_opts.material3 and user_opts.material3_surface_radius > 0 then
        lo.box.radius = user_opts.material3_surface_radius
    end
